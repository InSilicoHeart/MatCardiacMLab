%===============================================================================
% CellML file:   E:\My Dropbox\UniZar DropBox\Doctorado\CellML\Carro_Rodriguez_Laguna_Pueyo_2011_ENDO.cellml
% CellML model:  Carro_Rodriguez_Laguna_Pueyo_CinC2010_EPI
% Date and time: 21/01/2011 at 10:14:34
%-------------------------------------------------------------------------------
...
function [dY, CompVar] = Carro_Rodriguez_Laguna_Pueyo_2011_ENDO(t, Y, Constants, Values)

%-------------------------------------------------------------------------------
% Initial conditions
%-------------------------------------------------------------------------------

% Y = [6.138856e-1, 9.719632e-5, 2.048633e-4, 1.188246e-4, 1.262853, 3.288063e-4, 2.522168e-3, 1.369514e-1, 2.38683e-3, 1.225802e-1, 8.128604e-3, 9.811535e-3, 7.126555e-1, 7.119893e-1, 2.176608e-3, 8.078504e-2, 1.238366e-1, 8.606485e-3, 1.101044e-2, 1.879996e-6, 9.789409e-1, 9.993986e-1, 2.861794e-2, 1.69833e-2, 1.896559e-2, 1.43831e-7, 1.149876e-6, 8.888214e-1, 3.55636e-3, 3.785209e0, 8.259271e-1, 1.001989e1, 1.00211e1, 1.001974e1, 3.592405e-4, 3.592503e-4, 9.999976e-1, 8.161309e-1, -8.410546e1];
...
% YUnits = {'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'dimensionless', 'dimensionless', 'dimensionless', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'millimolar', 'dimensionless', 'dimensionless', 'dimensionless', 'dimensionless', 'millivolt'};
...
G_to_f = 1.404e-3;   % milliS_per_microF (in Transient_Outward_K_Current)
G_to_s = 3.7596e-2;   % milliS_per_microF (in Transient_Outward_K_Current)
...
Ibar_Ca_j = p_Ca*Y(39)*Frdy*FoRT*(Y(3)*exp(2.0*Y(39)*FoRT)-Ca_o)/(exp(2.0*Y(39)*FoRT)-1.0);
...
Ibar_Ca_sl = p_Ca*Y(39)*Frdy*FoRT*(Y(4)*exp(2.0*Y(39)*FoRT)-Ca_o)/(exp(2.0*Y(39)*FoRT)-1.0);
...
I_K1 = G_K1*sqrt(K_o/5.4)*K1_ss*(Y(39)-E_K);
...
Ibar_K = p_K*Y(39)*Frdy*FoRT*(K_i*exp(Y(39)*FoRT)-K_o)/(exp(Y(39)*FoRT)-1.0);
Ibar_Na_j = p_Na*Y(39)*Frdy*FoRT*(Y(33)*exp(Y(39)*FoRT)-Na_o)/(exp(Y(39)*FoRT)-1.0);
Ibar_Na_sl = p_Na*Y(39)*Frdy*FoRT*(Y(34)*exp(Y(39)*FoRT)-Na_o)/(exp(Y(39)*FoRT)-1.0);
...
I_NaK_junc = F_junc*Ibar_NaK*f_NaK*K_o/((1.0+(Km_Naip/Y(33))^4.0)*(K_o+Km_Ko));
I_NaK_sl = F_sl*Ibar_NaK*f_NaK*K_o/((1.0+(Km_Naip/Y(34))^4.0)*(K_o+Km_Ko));
I_NaK = I_NaK_junc+I_NaK_sl;
E_Ks = 1.0/FoRT*log((K_o+p_Na_K*Na_o)/(K_i+p_Na_K*Y(32)));
kp_Kp = 1.0/(1.0+exp(7.488-Y(39)/5.98));
I_Kp = G_Kp*kp_Kp*(Y(39)-E_K);
I_to_s = G_to_s*Y(36)*Y(38)*(Y(39)-E_K);
I_to_f = G_to_f*Y(35)*Y(37)*(Y(39)-E_K);
I_to = I_to_s+I_to_f;
r_Kr = 1.0/(1.0+exp((Y(39)+74.0)/24.0));
I_Kr = G_Kr*sqrt(K_o/5.4)*Y(25)*r_Kr*(Y(39)-E_K);
I_Ks_junc = F_junc*G_Ks_junc*Y(29)^2.0*(Y(39)-E_Ks);
I_Ks_sl = F_sl*G_Ks_sl*Y(29)^2.0*(Y(39)-E_Ks);
I_Ks = I_Ks_junc+I_Ks_sl;
I_K_tot = I_to+I_Kr+I_Ks+I_K1-2.0*I_NaK+I_Ca_K+I_Kp;
